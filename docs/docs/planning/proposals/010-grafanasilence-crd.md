...
Unlike other grafana entities which are assigned a UID by Grafana, silences are assigned a random ID by Grafana when created. This means that the operator must track the ID of the silence it created in order to update or delete it later. The operator stores this ID per instance in `status.silenceIDs` (a map of `<instance namespace>/<instance name>` to silence ID), so that it can reconcile updates and deletions correctly.

Because `status` is a protected subresource, a user cannot set it on creation through a normal apply, which rules out using `status.silenceIDs` alone to adopt/own an existing silence entry. To support adoption, the annotation `grafana.integreatly.org/adopt-silence-id` accepts the same map shape and is used as a one-time seed: on reconcile, any instance key present in the annotation but missing from `status.silenceIDs` is copied into it. Once `status.silenceIDs` tracks a given instance, the annotation is ignored for that key, even if it is edited afterwards, so `status.silenceIDs` remains the single source of truth for normal reconciliation. A seeded ID that turns out to be missing or expired is simply replaced with a newly created silence.
