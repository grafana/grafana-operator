...
func resolverWithVariables(variables ...v1beta1.GrafanaContentVariable) *Resolver {
	dashboard := &v1beta1.GrafanaDashboard{
		...
		Spec: v1beta1.GrafanaDashboardSpec{
			GrafanaContentSpec: v1beta1.GrafanaContentSpec{Variables: variables},
		},
	}
...
func TestResolveVariables(t *testing.T) {
	t.Run("overrides a datasource variable without options", func(t *testing.T) {
		...
		resolverWithVariables(v1beta1.GrafanaContentVariable{Name: "datasource", Value: "prometheus-uid"}).overrideVariables(model)
	...
	t.Run("reconciles options when the value already exists", func(t *testing.T) {
		...
		resolverWithVariables(v1beta1.GrafanaContentVariable{Name: "namespace", Value: "default"}).overrideVariables(model)
	...
	t.Run("synthesizes an option when the value is not present", func(t *testing.T) {
		...
		resolverWithVariables(v1beta1.GrafanaContentVariable{Name: "cluster", Value: "b"}).overrideVariables(model)
	...
	t.Run("ignores a variable that is not present in the model", func(t *testing.T) {
		...
		resolverWithVariables(v1beta1.GrafanaContentVariable{Name: "does-not-exist", Value: "x"}).overrideVariables(model)
	...
	t.Run("no-op when templating block is absent", func(t *testing.T) {
		model := map[string]any{}
		require.NoError(t, json.Unmarshal([]byte(`{"title": "no templating"}`), &model))

		// Must not panic.
		resolverWithVariables(v1beta1.GrafanaContentVariable{Name: "namespace", Value: "x"}).overrideVariables(model)
