...
func (r *IngressReconciler) getHTTPRouteAdminURL(ctx context.Context, httpRoute *gwapiv1.HTTPRoute) (adminURL string) {
...
func (r *IngressReconciler) getMatchListener(ctx context.Context, httpRoute *gwapiv1.HTTPRoute, gw *gwapiv1.Gateway) *gwapiv1.Listener {
	log := logf.FromContext(ctx)
	...
	for _, listener := range gw.Spec.Listeners {
		if listener.AllowedRoutes != nil && listener.AllowedRoutes.Namespaces != nil {
			...
			case gwapiv1.NamespacesFromAll:
				if len(hostnames) == 0 || listener.Hostname != nil && slices.Contains(hostnames, *listener.Hostname) {
					return &listener
				}
			case gwapiv1.NamespacesFromSelector:
				...
				if len(nsList.Items) > 0 {
					for _, item := range nsList.Items {
						if item.Name == httpRoute.Namespace && labelsSatisfyMatchExpressions(item.Labels,
			...
			case gwapiv1.NamespacesFromNone:
				continue
			}
		}
	}
...
func getHTTPRouteSpec(cr *v1beta1.Grafana, scheme *runtime.Scheme) gwapiv1.HTTPRouteSpec {
	...
	serviceName := gwapiv1.ObjectName(service.GetName())
	serviceNamespace := gwapiv1.Namespace(service.GetNamespace())
	servicePort := gwapiv1.PortNumber(port.IntVal)
	...
	return gwapiv1.HTTPRouteSpec{
		...
		Rules: func(cr *v1beta1.Grafana) []gwapiv1.HTTPRouteRule {
			{
				rules := make([]gwapiv1.HTTPRouteRule, 0, len(cr.Spec.HTTPRoute.Spec.Rules))
				for _, rule := range cr.Spec.HTTPRoute.Spec.Rules {
					rule.BackendRefs = []gwapiv1.HTTPBackendRef{
						{
