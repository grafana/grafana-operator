...
func (r *ReconcileGrafanaDataSource) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	fmt.Printf("Reconciling datasource: %s", request.NamespacedName)
...
func (r *ReconcileGrafanaDataSource) reconcileDatasource(cr *i8ly.GrafanaDataSource) (reconcile.Result, error) {
	ds, err := i8ly.ParseDataSource(cr)
	if err != nil {
		log.Error(err, "error parsing datasource")
		return reconcile.Result{}, err
	}

	changed := r.hasDatasourceChanged(ds, cr)
	if !changed {
		return reconcile.Result{Requeue: false}, nil
	}

	updateResponse, err := r.grafanaHelper.UpdateDataSource(context.TODO(), cr)
	if err != nil {
		return reconcile.Result{}, err
	}
	...
	err = r.helper.RestartGrafanaIfNeeded()
	if err != nil {
		log.Error(err, "error restarting grafana")
	}

	log.Info(fmt.Sprintf("datasource '%s' updated", cr.Spec.DataSource.Name))

	return r.updatePhase(cr, common.StatusResourceCreated)
}

func (r *ReconcileGrafanaDataSource) DeleteDatasource(cr *i8ly.GrafanaDataSource) (reconcile.Result, error) {
	_, err := r.grafanaHelper.DeleteDataSource(context.TODO(), cr)
	if err != nil {
	...
	log.Info(fmt.Sprintf("datasource '%s' deleted", cr.Spec.DataSource.Name))
	return reconcile.Result{}, err
}
...
func (r *ReconcileGrafanaDataSource) hasDatasourceChanged(newConfig string, d *i8ly.GrafanaDataSource) bool {
	hash := fmt.Sprintf("%x", md5.Sum([]byte(newConfig)))
	changed := hash != d.Status.LastConfig

	if !changed {
		known, err := r.grafanaHelper.DataSourceIsKnown(context.TODO(), d)
		if err != nil {
			log.Error(err, "error checking datasource status")
			return false
		}
		...
		// import it again
		if known {
			log.Info("datasource reconciled but no changes")
			return false
		}
	}
