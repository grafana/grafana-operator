...
| watchLabeledReferencesOnly | bool | `false` | Sets the `WATCH_LABELED_REFERENCES_ONLY` environment variable, Only applicable when env var EXPERIMENTAL_ENABLE_CACHE_LABEL_LIMITS="1" is set. When cache limits are enabled ConfigMaps and Secrets are no longer cached, this will trigger more api requests when CRs reference either. this re-enables caching of ConfigMaps and Secrets, labeled with `"app.kubernetes.io/managed-by": "grafana-operator"` to reduce to api requests. WARNING This hides unlabeled ConfigMaps and Secrets from the Operator. |
